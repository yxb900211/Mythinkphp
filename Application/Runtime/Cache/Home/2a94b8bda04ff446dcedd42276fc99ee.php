      ...
      if(!first){
        $.get('/index.php/Home/Index/index?p='+obj.curr,function(data){
        	...
        	$.each(data.db,function(i,item){
        		$('tbody').append('<tr><td>'+item.id+'</td><td>'+item.username+'</td><td></td><td></td><td>'+item.add_time+'</td><td><a href="/index.php/Home/Index/edit/id/<?php echo ($db["id"]); ?>"><button>修改</button></a> <a class="ajax-get" href="/index.php/Home/Index/delete/id/<?php echo ($db["id"]); ?>/_token/<?php echo ($token); ?>"><button>删除</button></a></td></tr>');
        	});
        });
      }
    }
